...
def glad_l2r(ncf, output = None, ofile = None,
    ...
    ## recompute orange band
        if (sensor in ['L8_OLI', 'L9_OLI']):
            ...
            if sensor == 'L8_OLI':
                ob_cfg = ac.shared.import_config(ac.config['directory']['data']+'/Landsat/L8_OLI_orange.cfg')
                sensor_o = 'L8_OLI_ORANGE'
            if sensor == 'L9_OLI':
                ob_cfg = ac.shared.import_config(ac.config['directory']['data']+'/Landsat/L9_OLI_orange.cfg')
