...
def reproject2(data, lon0, lat0, lon1, lat1, fill=-9999, nearest=True):
    ...
    if nearest:
        source_con = image.ImageContainerNearest(data,source_def,radius_of_influence=100)
        target_con = source_con.resample(target_def)
        target_con.fill_value=fill
        result = target_con.image_data
    else:
        from pyresample import kd_tree
        result = kd_tree.resample_gauss(source_def, data, target_def, radius_of_influence=100,neighbours=1,sigmas=0,
                                       fill_value=fill)
