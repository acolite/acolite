...
def qaa_compute(qaa_in, sza = 0, satellite = None,
    ...
    rhow_v5=np.log10((qaa_data['rrs443']+qaa_data['rrs490'])/\
                        (qaa_data['rrs560']+(5.*qaa_data['rrs665']*(qaa_data['rrs665']/qaa_data['rrs490']))))
    qaa_data['a560_v5'] = qaa_coef['aw'][2]+np.power(10.,(qaa_coef['h'][2]+(qaa_coef['h'][1]*rhow_v5)+qaa_coef['h'][0]*np.power(rhow_v5,2.)))

    ## get rhow and a665 according to QAA6
    rhow_v6=qaa_data['rrs665']/(qaa_data['rrs443']+qaa_data['rrs490'])
    qaa_data['a665_v6']= qaa_coef['aw'][3]+qaa_coef['k'][0]*np.power(rhow_v6,qaa_coef['k'][1])

    ## step 3
    qaa_data['bbp560_v5'] = ((qaa_data['u560'] * qaa_data['a560_v5']) / (1.0 - qaa_data['u560'])) - qaa_coef['bbw'][2]
    qaa_data['bbp665_v6'] = ((qaa_data['u665'] * qaa_data['a665_v6']) / (1.0 - qaa_data['u665'])) - qaa_coef['bbw'][3]
    ...
    for version in ['5','6']:
        ...
        for iw,wave in enumerate(qaa_wave):
            ## get dataset tags
            bbptag = 'bbp{}_v{}'.format(wave, version)
            atag = 'a{}_v{}'.format(wave, version)
            utag = 'u{}'.format(wave)
            kdtag = 'Kd{}_v{}'.format(wave, version)

            ## compute bbp and q
            if not (((version == '5') & (wave == 560)) or ((version == '6') & (wave == 665))):
                ##step 5 - get bbp
                qaa_data[bbptag] = qaa_data['bbp{}_v{}'.format(refn,version)] * \
