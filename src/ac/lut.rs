...
impl LutManager {
    ...
    pub fn ensure_dir(&self) -> Result<()> {
        fs::create_dir_all(&self.lut_dir).map_err(|e| AcoliteError::Io(e))
    }
    ...
    pub fn download_lut(&self, name: &str, url: &str) -> Result<()> {
        ...
        let size = bytes.len();
        fs::write(self.lut_path(name), bytes).map_err(|e| AcoliteError::Io(e))?;
    ...
    pub fn load_rayleigh_lut(&self) -> Result<RayleighLut> {
        ...
        let content = fs::read_to_string(path).map_err(|e| AcoliteError::Io(e))?;
...
impl RayleighLut {
    ...
    pub fn interpolate(
        ...
        interp_lut_1d(
            wavelength,
            &self.wavelengths,
            &self.reflectance[0..self.wavelengths.len()],
        )
    }
}
...
pub fn interp_lut_2d(x: f64, y: f64, x_vals: &[f64], y_vals: &[f64], z_vals: &[Vec<f64>]) -> f64 {
    let x_interp: Vec<f64> = y_vals
        .iter()
        .enumerate()
