...
impl GenericAerosolLut {
    ...
    pub fn romix(&self, wave_um: f64, pressure: f64, azi: f64, thv: f64, ths: f64, tau: f64) -> f64 {
        self.par_rgi.get(&PAR_ROMIX)
            .map(|rgi| rgi.interpolate(&[pressure, wave_um, azi, thv, ths, tau]))
            .unwrap_or(f64::NAN)
    }
    pub fn astot(&self, wave_um: f64, pressure: f64, azi: f64, thv: f64, ths: f64, tau: f64) -> f64 {
        self.par_rgi.get(&PAR_ASTOT)
            .map(|rgi| rgi.interpolate(&[pressure, wave_um, azi, thv, ths, tau]))
            .unwrap_or(f64::NAN)
    }
    pub fn dutott(&self, wave_um: f64, pressure: f64, azi: f64, thv: f64, ths: f64, tau: f64) -> f64 {
        self.par_rgi.get(&PAR_DUTOTT)
            .map(|rgi| rgi.interpolate(&[pressure, wave_um, azi, thv, ths, tau]))
            .unwrap_or(f64::NAN)
    }
    ...
    pub fn romix_spectrum(&self, pressure: f64, azi: f64, thv: f64, ths: f64, tau: f64) -> Vec<f64> {
        self.wave_um.iter().map(|&w| self.romix(w, pressure, azi, thv, ths, tau)).collect()
    }

    /// Query all three parameters at all LUT wavelengths.
    /// Returns (romix_vec, astot_vec, dutott_vec), each of length wave_um.len().
    pub fn params_spectrum(&self, pressure: f64, azi: f64, thv: f64, ths: f64, tau: f64) -> (Vec<f64>, Vec<f64>, Vec<f64>) {
...
pub fn rsr_convolve_gauss(
    ...
    for (i, &w) in lut_wave_um.iter().enumerate() {
        if (w - center_um).abs() > half_width { continue; }
        let response = (-(w - center_um).powi(2) / (2.0 * sigma * sigma)).exp();
        if response < 0.0025 { continue; } // min_sensitivity threshold
...
pub fn load_generic_lut(
    ...
    let par_names: Vec<String> = {
        ...
        match val {
            ...
            _ => return Err(AcoliteError::Processing("Unexpected 'par' attribute type".into())),
        }
    };
    ...
    for &pres in pressures {
        ...
        if !path.exists() {
            return Err(AcoliteError::Processing(format!("Generic LUT not found: {}", path.display())));
        }
        ...
        let var = nc.variable("lut")
            .ok_or_else(|| AcoliteError::Processing("Missing 'lut' variable in generic LUT".into()))?;
        ...
        for iw in 0..nwave {
            for ia in 0..nazi {
                for iv in 0..nthv {
                    for is_ in 0..nths {
                        for it in 0..ntau {
                            let base = iw * stride_wave + ia * stride_azi + iv * stride_thv + is_ * stride_ths + it;
    ...
    let axes = vec![pres_axis.clone(), wave_um.clone(), azi.clone(), thv.clone(), ths.clone(), tau.clone()];

    let mut par_rgi = HashMap::new();
    par_rgi.insert(PAR_ROMIX, RegularGridInterpolator::new(axes.clone(), romix_data));
    par_rgi.insert(PAR_ASTOT, RegularGridInterpolator::new(axes.clone(), astot_data));
    par_rgi.insert(PAR_DUTOTT, RegularGridInterpolator::new(axes, dutott_data));

    log::info!("Loaded generic LUT {}: {}×{} wave×tau, {} pressures", base_name, nwave, ntau, npres);
...
fn download_generic_lut(lut_subdir: &Path, base_name: &str, pressures: &[f64]) -> Result<()> {
    ...
    for &pres in pressures {
        ...
        let response = client.get(&bz2_url).send()
            .map_err(|e| AcoliteError::Processing(format!("Download LUT: {}", e)))?;

        if !response.status().is_success() {
            return Err(AcoliteError::Processing(format!("Download LUT failed: HTTP {}", response.status())));
        }

        let bz2_data = response.bytes()
            .map_err(|e| AcoliteError::Processing(format!("Read LUT: {}", e)))?;

        // Decompress bz2
        let mut decoder = bzip2::read::BzDecoder::new(&bz2_data[..]);
        let mut nc_data = Vec::new();
        decoder.read_to_end(&mut nc_data)
...
#[cfg(feature = "full-io")]
pub fn load_generic_luts(
    data_dir: &Path,
    pressures: &[f64],
) -> Result<Vec<GenericAerosolLut>> {
    ...
    if luts.is_empty() {
        return Err(AcoliteError::Processing("No generic aerosol LUTs loaded".into()));
    }
    Ok(luts)
}
