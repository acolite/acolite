...
pub fn compute_gas_transmittance(
    ...
    for band in &band_names {
        if let Some(rsr) = rsr_map.get(band) {
            ...
            let band_wave_nm = rsr_convolve(&rsr.wave, &rsr.wave, rsr); // effective wavelength
...
pub fn compute_wv_transmittance(wave_nm: f64, uwv: f64, mu0: f64, muv: f64) -> f64 {
    ...
    let k_wv = if wave_nm < 500.0 {
        0.0
    } else if wave_nm < 600.0 {
        0.0
...
fn read_nc_attr_f64_vec(ds: &netcdf::File, name: &str) -> Result<Vec<f64>> {
        ...
        .map_err(|e| AcoliteError::Processing(format!("Read attr '{}': {}", name, e)))?
    {
        ...
        other => Err(AcoliteError::Processing(format!(
            "Unexpected type for '{}': {:?}",
            name, other
        ))),
    }
...
pub fn load_wv_lut(data_dir: &Path) -> Result<WvLut> {
    ...
    let ds =
        netcdf::open(&path).map_err(|e| AcoliteError::Processing(format!("Open WV LUT: {}", e)))?;
...
pub fn load_gas_lut(data_dir: &Path) -> Result<GasLut> {
    ...
    let par_str: String = match par_attr
        .value()
        .map_err(|e| AcoliteError::Processing(e.to_string()))?
    {
...
pub fn compute_gas_transmittance_hyper(
    ...
    let tt_o3_hyper: Vec<f64> = ko3_data
        .iter()
        .map(|&k| {
            let tau = k * uoz;
    ...
    let wv_on_lut: Vec<f64> = wv_lut
        .wave
        .iter()
        .map(|&w| wv_lut.rgi.interpolate(&[sza, vza, uwv, wv_par_id, w]))
        .collect();
    // Interpolate to ko3 grid
    let tt_wv_hyper: Vec<f64> = ko3_wave_um
        .iter()
        .map(|&w| interp_1d(w, &wv_lut.wave, &wv_on_lut))
        .collect();

    // ── Compute other gas transmittances on Gas LUT grid, then interpolate to ko3 grid ──
    let par_idx: HashMap<&str, usize> = gas_lut
        .par_names
        .iter()
        .enumerate()
        .map(|(i, n)| (n.as_str(), i))
    ...
    for &par in &gas_pars {
        ...
        let vals_on_lut: Vec<f64> = gas_lut
            .wave
            .iter()
            .map(|&w| gas_lut.rgi.interpolate(&[pressure, pidx, w, vza, sza]))
            .collect();
        // Interpolate to ko3 grid
        let vals_on_ko3: Vec<f64> = ko3_wave_um
            .iter()
            .map(|&w| interp_1d(w, &gas_lut.wave, &vals_on_lut))
            .collect();
